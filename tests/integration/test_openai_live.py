...
def _openai_auth():
    auth = resolve_env_auth("openai", env=os.environ)
...
async def test_openai_tool_call_and_tool_result_round_trip_live() -> None:
    ...
    _print_usage("openai tool call alpha", first_tool_response)
    _print_usage("openai tool result alpha", first_answer_response)
    _print_usage("openai tool call beta", second_tool_response)
